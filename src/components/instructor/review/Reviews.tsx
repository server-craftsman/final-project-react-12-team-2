  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      {filteredReviews.map((review: Review) => (
        <div key={review.id} className="transform rounded-lg bg-white p-6 shadow-lg transition duration-500 hover:scale-105">
          <h3 className="mb-2 text-xl font-semibold">{coursesData.courses.find((course) => course.id === review.course_id)?.name}</h3>
