  ...
  const columns: ColumnsType<Review> = [
    {
      ...
      onCell: (_, index) => {
        if (
          index === undefined ||
          !searchedReviews ||
          index >= searchedReviews.length
        )
          return { rowSpan: 0 }; // Ensure index is defined and within bounds
        const courseId = searchedReviews[index].course_id;
        const courseReviews = searchedReviews.filter(
          (review) => review.course_id === courseId,
        );
        const rowSpan =
          index === 0 || searchedReviews[index - 1].course_id !== courseId
    ...
    },
    {
      ...
      onCell: (_, index) => {
        if (
          index === undefined ||
          !searchedReviews ||
          !Array.isArray(searchedReviews)
        )
          return { rowSpan: 0 }; // Ensure searchedReviews is defined and an array
        const courseId = searchedReviews[index].course_id;
        const courseReviews = searchedReviews.filter(
          (review) => review.course_id === courseId,
        );
