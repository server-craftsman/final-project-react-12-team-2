...
const DisplayCourse = ({ searchTerm, statusFilter, onSearch, onStatusChange, refreshKey }: { searchTerm: string; statusFilter: StatusType; onSearch: (value: string) => void; onStatusChange: (status: StatusType | "") => void; refreshKey: number }) => {
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      key: "price",
      dataIndex: "price",
      render: (price: number) => <div className="flex justify-end font-semibold text-emerald-600">{moneyFormat(price)}</div>
    },
    {
      title: "Discount",
      key: "discount",
    ...
    },
    {
      ...
      render: (text: Date) => <span className="font-bold">{formatDate(text)}</span>
    },
    {
      title: "Actions",
      key: "actions",
