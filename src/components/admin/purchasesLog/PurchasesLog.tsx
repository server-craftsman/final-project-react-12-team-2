...
const PurchasesLog: React.FC<PurchasesLogProps> = ({ searchQuery, statusFilter }) => {
  const [purchasesLogData] = useState<Purchases[]>(purchaseLogData.purchases as unknown as Purchases[]);
  ...
  useEffect(() => {
    ...
    const filtered = purchasesWithCourseNameAndStudentName.filter((purchase) => {
      const matchesSearchQuery = Object.values(purchase).some((value) => typeof value === "string" && value.toLowerCase().includes(searchQuery.toLowerCase()));
