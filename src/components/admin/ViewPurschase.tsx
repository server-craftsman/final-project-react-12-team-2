...
import { Purchases, PurchaseStatusEnum } from '../../models/Purchase'; 
...
const ViewPurchase = ({ searchQuery }: { searchQuery: string }) => {
  ...
  const filteredPurchases = convertedPurchasesData.filter((purchase: Purchases) =>
  ...
  const columns = [
    ...
    },
    {
      ...
      onFilter: (value: string | Key, record: Purchases) => record.status === value,
