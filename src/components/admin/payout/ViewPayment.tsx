...
const ViewPayment: React.FC<ViewPaymentProps> = ({ searchQuery, status }) => {
  ...
  useEffect(() => {
    const filteredPayments: Payout[] = paymentsData.payments
      .filter((payment) => payment.payout_no.toLowerCase().includes(searchQuery.toLowerCase()) || payment.instructor_id.toLowerCase().includes(searchQuery.toLowerCase()))
      .map((payment) => ({
        ...payment,
  ...
  const handleApprove = (id: string, newStatus: PayoutStatusEnum) => {
    Modal.confirm({
      title: `Are you sure you want to ${newStatus === PayoutStatusEnum.completed ? "approve" : "reject"} this payment?`,
      onOk: () => {
        setPayments((prevPayments) => prevPayments.map((payment) => (payment.id === id ? { ...payment, status: newStatus } : payment)));
        message.success(`Payment ${newStatus === PayoutStatusEnum.completed ? "approved" : "rejected"} successfully.`);
      },
      onCancel: () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: Payout) => (
        <button onClick={() => handleViewDetails(record.id)} className="bg-gradient-tone rounded-md px-4 py-2 text-white" style={{ width: "110px", textAlign: "left" }}>
          View Details
        </button>
      )
    },
    ...
    },
    {
      ...
      dataIndex: "status",
      key: "status",
      render: (status: PayoutStatusEnum) => <Tag color={status === PayoutStatusEnum.completed ? "green" : status === PayoutStatusEnum.request_payout ? "orange" : "volcano"}>{PayoutStatusEnum[status.toLowerCase() as keyof typeof PayoutStatusEnum]}</Tag>
    },
    {
      title: "Date Created",
      ...
      render: (_: unknown, record: Payout) => (
        <div className="flex items-center justify-between" style={{ minHeight: "48px" }}>
          {record.status === PayoutStatusEnum.request_payout && (
            <>
              <button onClick={() => handleApprove(record.id, PayoutStatusEnum.completed)} className="rounded-md bg-blue-500 px-4 py-2 text-white" style={{ width: "100px" }}>
                Approve
              </button>
              <button onClick={() => handleApprove(record.id, PayoutStatusEnum.rejected)} className="ml-1 rounded-md bg-red-500 px-4 py-2 text-white" style={{ width: "70px" }}>
                Reject
              </button>
            </>
          )}
  ...
  return (
    <div className="p-0">
      <Table<Payout> className="shadow-lg" columns={columns} dataSource={payments} rowKey="id" pagination={{ pageSize: 10 }} rowClassName={() => "align-middle"} />

      <Modal title="Payout Details" open={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null} width={1000}>
        <Table columns={detailColumns} dataSource={selectedPayoutDetails} rowKey="id" pagination={false} />
      </Modal>
    </div>
  );
};
