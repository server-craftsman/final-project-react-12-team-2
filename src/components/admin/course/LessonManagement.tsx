...
const LessonManagement: React.FC<LessonManagementProps> = ({ searchTerm }) => {
  ...
  const filteredLessons = lessons.filter((lesson) => lesson.name.toLowerCase().includes(searchTerm.toLowerCase()) || getCoursesNameBySessionId(lesson.course_id).toLowerCase().includes(searchTerm.toLowerCase()));
