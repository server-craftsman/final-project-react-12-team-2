...
const CoursesManagement: React.FC<{
  ...
  const [coursesData, setCourses] = useState<Course[]>(courseData.courses as unknown as Course[]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status: CourseStatusEnum) => (
        <span className="text-sm capitalize" style={{ color: courseStatusColor[status] }}>
    ...
    },
    {
      ...
      render: (record: Course) => (
        <div>
          <CheckOutlined style={{ color: "green", marginRight: 8, cursor: "pointer" }} onClick={() => handleApprove(record.id)} />
          <Popconfirm title="Block the course?" description="Are you sure to block this course?" onConfirm={() => handleBlock(record.id)} okText="Yes" cancelText="No">
  ...
  const filteredCourses = coursesData.filter((course) => course.name.toLowerCase().includes(searchTerm.toLowerCase()) && (statusFilter === "" || course.status === statusFilter));
