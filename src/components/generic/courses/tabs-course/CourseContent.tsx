    ...
    >
      {sessions.map((session) => (
        <Collapse
          ...
          onChange={(key) =>
            setActiveSessionId(Array.isArray(key) ? key[0] : key)
          }
          items={[
            {
              key: session.id,
              label: (
                <Title
                  level={4}
                  className="mb-0 border-l-4 border-[#8529ff] pl-4 text-2xl font-bold text-[#1a237e]"
                >
                  {session.name}
                </Title>
              ),
              children: (
                <div className="flex flex-col items-start justify-start gap-6 rounded-lg border border-gray-300 bg-white p-12 shadow-2xl">
                  <Paragraph className="text-xl leading-relaxed text-gray-800">
                    {session.description}
                  </Paragraph>
                  <List
                    dataSource={lessons
                      ...
                      <List.Item className="flex flex-row items-start justify-start text-left">
                        ...
                        >
                          <Button
                            type="link"
                            block
                            className="flex items-center text-left text-xl text-[#1a237e] hover:text-[#8529ff]"
                          >
