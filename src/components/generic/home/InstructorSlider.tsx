...
const InstructorSlider = () => {
  ...
  return (
    <div className="relative mx-auto max-w-6xl rounded-lg bg-gradient-to-r from-indigo-900 to-purple-900 px-4 py-12 shadow-xl">
      <h2 className="mb-8 text-center text-3xl font-bold tracking-wide text-white">
      ...
      >
        {instructors.map((instructor, index: number) => (
          <SwiperSlide key={instructor.id}>
            <div
              className={`flex flex-col items-center transition-all duration-300 ${index === activeIndex ? "z-10 scale-105" : "scale-95 opacity-80"}`}
            >
              <div className="relative mb-4">
                <img
                  src={
                    instructor.avatar_url || "https://via.placeholder.com/150"
                  }
                  alt={instructor.name}
                  className="border-gold h-24 w-24 rounded-full border-2 object-cover shadow-md"
                />
                <div className="from-gold absolute inset-0 rounded-full bg-gradient-to-br to-amber-300 opacity-20"></div>
              </div>
              <div
                className={`text-center transition-all duration-300 ${index === activeIndex ? "opacity-100" : "opacity-0"}`}
              >
                <h3 className="mb-1 text-xl font-semibold text-white">
                  {instructor.name}
                </h3>
                <p className="mb-2 text-xs italic text-gray-300">
